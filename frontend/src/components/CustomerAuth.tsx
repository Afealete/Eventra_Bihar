...
import { firebaseGoogleLogin, login, logout, signup, me, CurrentUser, PublicAuthRole } from "../lib/auth";
...
export default function CustomerAuth({ defaultMode }: AuthProps) {
  ...
  const [role, setRole] = useState<PublicAuthRole>("customer");
  ...
  useEffect(() => {
    ...
    (async () => {
      setLoading(true);
      const data = await me();
      if (!mounted || signInInProgress.current) return;
  ...
  const redirectFromStoredRole = async (authenticated: { user: CurrentUser }) => {
    if (authenticated.user.role === "admin") {
      await logout();
      throw new Error("Use the dedicated admin sign-in page for administrator access.");
    }
    router.replace(authenticated.user.role === "vendor" ? "/vendor/dashboard" : "/");
  };
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      await redirectFromStoredRole(await firebaseGoogleLogin(role));
    } catch (err: any) {
  ...
  const handleEmailAuth = async (values: any, actions: any) => {
    ...
    signInInProgress.current = true;
    try {
      let authenticated: { user: CurrentUser };
      if (mode === "login") {
        authenticated = await login(values.email, values.password);
      } else {
        authenticated = await signup(values.email, values.password, values.name || undefined, role);
      }
      await redirectFromStoredRole(authenticated);
  ...
  return (
    ...
    >
      ...
      {firebaseError && <p role="alert" className="mb-4 rounded-lg bg-red-50 p-3 text-sm text-red-800">{firebaseError}</p>}
      ...
      >
        {({ isSubmitting }) => (
          <Form className="space-y-4">
            {mode === "signup" && (
              <div>
                <label htmlFor="auth-name" className="mb-1 block text-sm font-medium">Full name</label>
                <Field id="auth-name"
                  name="name"
            ...
            <div>
              <label htmlFor="auth-email" className="mb-1 block text-sm font-medium">Email address</label>
              <Field id="auth-email"
                name="email"
                type="email"
            ...
            <div>
              ...
              <Field id="auth-password"
                name="password"
                type={showPassword ? "text" : "password"}
                placeholder="Password"
                className="w-full px-4 py-3 border rounded-lg bg-gray-50"
              />
              <button type="button" onClick={() => setShowPassword((value) => !value)} className="absolute right-3 top-3 text-sm font-semibold text-[#8B000F]" aria-label={showPassword ? "Hide password" : "Show password"}>{showPassword ? "Hide" : "Show"}</button>
              </div>
              <ErrorMessage
                name="password"
                component="div"
            ...
            {mode === "signup" && (
              <div>
                <label htmlFor="auth-confirm" className="mb-1 block text-sm font-medium">Confirm password</label>
                <Field id="auth-confirm"
                  name="confirm"
                  type={showPassword ? "text" : "password"}
            ...
            <div className="text-sm text-black/80">
              Signing in as: <strong className="text-black">{role}</strong>
            </div>
      ...
      <p className="mt-4 text-center text-sm text-black/80">Looking to manage a business? <a href="/vendor/auth" className="font-semibold text-[#8B000F]">Use the Vendor Portal</a>.</p>
    </AuthCard>
  );
}
